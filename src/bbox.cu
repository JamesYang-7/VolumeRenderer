...
bool AABB::isect() const{
    return true;
}

__host__ __device__ bool AABB::min_less_than_zero(const Ray* ray) const {
    return ray->origin.x < 0 || min.x < 0.0f || min.y < 0.0f || min.z < 0.0f;
};

__host__ __device__ void AABB::pointer_test(float* out) {
    *out = 1.0f;
}